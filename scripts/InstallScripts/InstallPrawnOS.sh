...
setup_users() {
    ...
    until chroot_wrapper passwd
    do
        echo "-----Enter a password for the root user-----"
        chroot_wrapper passwd
    done
    ...
    if [[ "$desktop" = "true" ]]; then
        ...
        until chroot_wrapper adduser "$username" --gecos \"\"
        do
            while true; do
                echo "-----Enter new username:-----"
                read -r username
        ...
        chroot_wrapper usermod -a -G sudo,netdev,input,video,bluetooth "$username"
    fi
